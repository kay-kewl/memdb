...
public:
    ...
    const std::map<RowID, Row>& get_all_rows() const;
    std::map<RowID, Row>& get_all_rows();
    ...
    void validate_row_update(const std::vector<std::optional<Value>>& updated_values, RowID current_row_id) const;
